...
| Native macOS app | **Experimental** | The native client is now a real operator surface with setup, dashboard, command-center/runtime views, memory, knowledge, files, people, finance, medical, email/calendar/todos, GitHub, native playbook proposal authoring/review/apply flows, full connection admin/remediation, and native governance parity for standing approvals, automation grants, read-only security policy, and vault summary. It still trails the web inspector for policy authoring, destructive vault administration, and a few deeper admin workflows, but it is no longer deferred. |
