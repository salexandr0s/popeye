...
Every request must include a `Bearer` token in the `Authorization` header. The token is validated against the auth store file specified by `config.authFile`. Token rotation is supported: during the overlap window, both the current and next tokens are accepted.
...
| Method | Path | Description |
|--------|------|-------------|
| GET | `/v1/instruction-previews/:scope` | Preview compiled instructions for a scope |
...
Connect to `GET /v1/events/stream` with a valid `Authorization: Bearer <token>` header. The connection returns `text/event-stream` and emits frames in standard SSE format:
