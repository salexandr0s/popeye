...
| Endpoint(s) | Native view / use | Read/Write | Min role | Live update | Readiness | Notes / gaps | Service |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `GET /v1/email/accounts`, `GET /v1/email/threads`, `GET /v1/email/threads/:id`, `GET /v1/email/threads/:id/messages`, `GET /v1/email/messages/:id`, `GET /v1/email/search`, `GET /v1/email/digest`, `GET /v1/email/drafts`, `GET /v1/email/drafts/:id`, `GET /v1/email/providers` | Native Mail split view with account picker, inbox/search modes, unread-only filtering, digest, draft list/reopen flow, thread detail, and thread-context compose seeding | Read | `operator` | Poll on screen | Ready with wrapper | Shipped as a daily-use native mail surface with explicit search + filter controls, persistent Popeye-managed draft retrieval, and reply/reply-all/forward draft seeding from synced thread messages; provider-side send still stays out of scope | `DomainDigestService` |
| `POST /v1/email/sync`, `POST /v1/email/digest`, `POST /v1/email/drafts`, `PATCH /v1/email/drafts/:id` | Native Mail toolbar, digest controls, and draft editor sheet actions | Write | `operator` | Refetch after success | Ready with wrapper | Shipped as targeted native mailbox sync, digest generation, Popeye-managed draft create/edit, and thread-context compose flows that save through the same draft endpoints | `DomainDigestService` |
...
| Email | Mail | **Shipped, search + targeted mutations** | Native now covers account-scoped search, unread filtering, mailbox sync, digest generation, Popeye-managed draft list/reopen/create/edit, and reply/reply-all/forward draft seeding in addition to the split view |
...
- `GET /v1/email/threads/:id/messages`
- `GET /v1/email/digest`
