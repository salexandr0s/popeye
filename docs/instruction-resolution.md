...
Instructions are sourced from 9 precedence levels, ordered lowest (applied first) to highest (applied last, highest priority):
...
| 6 | `identity` | Identity instructions (IDENTITY.md) |
| 7 | `task_brief` | Task-specific instructions |
| 8 | `trigger_overlay` | Trigger or event-specific overlays |
| 9 | `runtime_notes` | Runtime-injected notes and context |
...
5. Returns a `CompiledInstructionBundle` with a unique ID, the ordered sources, the compiled text, the bundle hash, any warnings, and a timestamp.
...
The runtime persists instruction bundles as snapshots in the `instruction_snapshots` table (keyed by scope) for audit and debugging. The control API exposes previews via `GET /v1/instruction-previews/:scope`.
