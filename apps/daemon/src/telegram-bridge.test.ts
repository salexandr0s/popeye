...
describe('telegram-bridge', () => {
  ...
  it('starts the Telegram bridge with a Popeye secret ref when env token is absent', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'popeye-telegram-secret-ref-'));
    chmodSync(dir, 0o700);
    const config = makeConfig(dir);
    config.telegram.secretRefId = 'secret-telegram-bot';
    ...
    const bridge = await startTelegramBridge(config, {
      createBotClient: () => bot,
      createControlClient: () => control,
      getSecretValue: (id) => (id === 'secret-telegram-bot' ? 'bot-token-from-secret' : null),
    });
    await vi.waitFor(() => {
      expect(control.ingestMessage).toHaveBeenCalledWith(expect.objectContaining({ workspaceId: 'default', text: 'hello' }));
      expect(bot.sendMessage).toHaveBeenCalledWith({ chatId: '777', text: 'done', replyToMessageId: 2 });
