...
import { useStandingApprovals } from '../api/hooks';
...
export function StandingApprovals() {
  const { data, error, loading, refetch } = useStandingApprovals();
  const api = useApi();
  ...
  const handleCreate = async () => {
    try {
      setCreateError(null);
      await api.post('/v1/policies/standing-approvals', {
        scope: form.scope,
        domain: form.domain,
      ...
      setCreateError(err instanceof Error ? err.message : 'Standing approval creation failed');
    }
  };
  ...
  const handleRevoke = async (id: string) => {
    try {
      setActionError(null);
      await api.post(`/v1/policies/standing-approvals/${id}/revoke`, { revokedBy: 'web_inspector' });
      refetch();
    } catch (err) {
      setActionError(err instanceof Error ? err.message : 'Standing approval revoke failed');
    }
  };
  ...
  const columns: Column<(typeof filtered)[number]>[] = [
    ...
    },
    {
      ...
      render: (row) => (
        <div>
          <p>{row.workspaceId ?? '*'}</p>
          <p className="text-[12px] text-[var(--color-fg-muted)]">{row.projectId ?? '*'}</p>
        </div>
      ),
    },
    {
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Create durable operator grants that can auto-resolve eligible approval requests."
      />

      <div className="mb-[24px] grid gap-[16px] md:grid-cols-3">
        <Card label="Total grants" value={data?.length ?? 0} description="Standing approvals stored by the runtime" />
        <Card label="Active" value={(data ?? []).filter((record) => record.status === 'active').length} description="Eligible to auto-resolve matching approvals" />
      ...
      <section className="mb-[24px] rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-surface)] p-[20px]">
        <h2 className="text-[16px] font-semibold">Create standing approval</h2>
        ...
        <div className="mt-[12px] flex items-center gap-[12px]">
          ...
          <p className="text-[12px] text-[var(--color-fg-muted)]">Created as <span className="font-mono">web_inspector</span>.</p>
        </div>
      ...
      {filtered.length === 0 ? (
        <EmptyState title="No standing approvals" description="Create one above or adjust the filters to see existing grants." />
      ) : (
