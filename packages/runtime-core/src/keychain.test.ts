...
describe('keychain', () => {
  ...
  describe('on non-darwin platforms', () => {
    ...
    it('isKeychainAvailable short-circuits without shelling out', () => {
      expect(isKeychainAvailable()).toBe(false);
      expect(mockSpawnSync).not.toHaveBeenCalled();
    });

    it('keychain operations return macOS-only errors without touching the filesystem or shell', () => {
      expect(keychainGet('missing')).toEqual({
        ok: false,
        error: 'keychain is only available on macOS',
      });
      expect(keychainSet('my-key', 'my-secret')).toEqual({
        ok: false,
        error: 'keychain is only available on macOS',
      });
      expect(keychainDelete('my-key')).toEqual({
        ok: false,
        error: 'keychain is only available on macOS',
      });
      expect(keychainHas('my-key')).toBe(false);
      expect(mockSpawnSync).not.toHaveBeenCalled();
